...
export const controlModules = ['canon_gates', 'representation_boundaries', 'uncertainty_boundaries', 'interpretation_sets'];
...
const BLOCKED_KEYS = new Set([
  'source_trace', 'audit_records', 'audit_workflow', 'provenance', 'phase_history',
  'batch_history', 'candidate_filename', 'validation_status', 'record_status',
  'runtime', 'runtime_retention', 'module_governance', 'technical_extension_paths',
  'internal_architecture_reference', 'source_filename', 'assembly_status', 'full_text'
]);

const PUBLIC_PRIORITY_KEYS = [
  'title', 'label', 'name', 'public_title', 'chapter_title', 'section_title',
  'entity_name', 'event_name', 'concept_label', 'claim_label', 'relation_label',
  'gate_label', 'set_label', 'type', 'category', 'summary', 'chapter_summary',
  'section_summary', 'description', 'plain_language_statement', 'claim_text',
  'boundary_statement', 'constraint', 'rule', 'interpretation_summary',
  'how_this_governs_interpretation', 'public_role'
];
...
export function publicFields(record, limit = 8) {
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    if (typeof value === 'string' && value.trim()) fields.push({ key: titleCase(key), value });
